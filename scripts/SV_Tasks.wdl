...
task Count_Final_Sample {
  ...
  command <<<
     ...
     bcftools query  -e  'INFO/SECONDARY=1' -f "[%ID\t%SAMPLE\t%GT\n]"  ~{input_vcf_gz} \
     | bgzip -c  > ~{output_name}
...
task Count_Final_Variant {
  ...
  command <<<
     ...
     bcftools query  -e  'INFO/SECONDARY=1' -f "%CHROM\t%POS\t%ID\t%ALT\t%FILTER\t%INFO/SVTYPE\t%INFO/SVLEN\t%INFO/AF\n"  ~{input_vcf_gz} \
     | bgzip -c   > ~{output_name}
...
task Smoove {
  ...
  runtime {
    ...
    disks: "local-disk " + ceil( size(input_cram, "GB") + size(input_cram_index, "GB") + size(ref_fasta, "GB") + size(ref_fasta_index, "GB") + size(exclude_regions, "GB") + size(input_cram, "GB") * 4 + size(ref_cache, "GB") * 5) + " HDD"
    preemptible: preemptible_tries
  }
